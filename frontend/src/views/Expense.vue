<template>
  <div class="expense">
    ...
    <el-card class="table-card">
      <el-table :data="expenseStore.expenses" v-loading="expenseStore.loading" style="width: 100%">
...
import { useExpenseStore, useCategoryStore, useAccountStore } from '../stores'
...
const handleFilterChange = () => {
  ...
  expenseStore.fetchExpenses(params)
}
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定删除这条支出记录吗？', '提示', {
      type: 'warning'
    })
    const res = await expenseStore.deleteExpense(row.id)
    if (res.success) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    let res
    if (isEdit.value) {
      res = await expenseStore.updateExpense(form.id, data)
    } else {
      res = await expenseStore.createExpense(data)
    }
